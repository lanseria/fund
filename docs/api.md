...
获取用于绘制基金净值曲线、RSI 指标曲线以及买卖信号点的全套数据。

-   **路径参数**:
    -   `fund_code` (string, **required**): 基金代码，例如 `004253`。
-   **查询参数**:
    -   `start_date` (string, **required**): 图表数据的开始日期，格式为 `YYYY-MM-DD`。

-   **示例请求**:
    ```bash
    curl -X GET "http://127.0.0.1:8888/charts/rsi/004253?start_date=2024-01-01"
    ```
...
-   **ECharts 前端使用建议**:
    *   `dates` 作为 `xAxis.data`。
    *   创建一个包含 `netValues` 和 `rsiValues` 的 `series`，分别对应两个 `yAxis`。
    *   使用 `rsiValues` 绘制主RSI曲线。
    *   使用 `config` 中的 `rsiUpper` 和 `rsiLower` 值，通过 `series.markLine` 绘制超买超卖的水平线。
    *   使用 `signals.buy` 和 `signals.sell` 中的数据，通过 `series.markPoint` 在RSI曲线上标记出买卖信号点。
